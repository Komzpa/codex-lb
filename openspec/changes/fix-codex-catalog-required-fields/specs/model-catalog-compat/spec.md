...
Every model entry returned by `GET /backend-api/codex/models` or the equivalent route MUST
include the non-defaulted Codex wire fields `truncation_policy` and
`experimental_supported_tools`, including the
`GET /v1/models?client_version=<version>` response, even
