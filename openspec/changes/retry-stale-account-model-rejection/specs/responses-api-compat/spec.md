...
The proxy MUST classify a Responses request as
`account_model_unsupported` when upstream rejects it with
`invalid_request_error` and the exact message `The '<model>' model is not
supported when using Codex with a ChatGPT account.` before accepting the
response. The quoted model MUST match
