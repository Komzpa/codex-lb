...
@pytest.mark.asyncio
async def test_reconnect_account_neutral_replay_does_not_re_require_excluded_owner(app_instance, monkeypatch):
    from app.modules.proxy.continuity import make_http_bridge_account_neutral_replay_key

    service = get_proxy_service_for_app(app_instance)
    recovery_kind, recovery_key = make_http_bridge_account_neutral_replay_key("reconnect-silent-account")
    first_account = cast(Account, SimpleNamespace(id="acct-silent", status=AccountStatus.ACTIVE, plan_type="plus"))
    replacement_account = cast(
        Account,
        SimpleNamespace(id="acct-replacement", status=AccountStatus.ACTIVE, plan_type="plus"),
    ...
    request_state = proxy_module._WebSocketRequestState(
        request_id="req-account-neutral-real-reconnect",
        model="gpt-5.5",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
    ...
    request_state.excluded_account_ids.add(first_account.id)
    selection_calls: list[dict[str, object]] = []
    ...
    async def fake_ensure_fresh_with_budget(self, account, *, force=False, timeout_seconds=None):
        del self, force, timeout_seconds
        return account

    async def fake_open_upstream_websocket_with_budget(self, account, headers, *, timeout_seconds, request_state):
        del self, headers, timeout_seconds, request_state
        assert account.id == replacement_account.id
        return replacement_upstream
    ...
    assert selection_calls == [
        {
            "preferred_account_id": None,
            "exclude_account_ids": {"acct-silent"},
            "preferred_account_is_continuity_owner": True,
            "fallback_on_preferred_account_unavailable": True,
        }
    ]
    assert session.account.id == replacement_account.id
