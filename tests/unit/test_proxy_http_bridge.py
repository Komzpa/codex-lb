...
async def test_http_bridge_precreated_proxy_anchor_keeps_hard_owner_without_alternative_probe(
    ...
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    fresh_text = '{"type":"response.create","model":"gpt-5.5","input":"full resend"}'
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-precreated-proxy-anchor-single-account",
        model="gpt-5.5",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        ...
        awaiting_response_created=True,
        event_queue=asyncio.Queue(),
        request_text='{"type":"response.create","model":"gpt-5.5","previous_response_id":"resp_proxy_anchor"}',
        transport="http",
        skip_request_log=True,
        affinity_policy=proxy_service._AffinityPolicy(
            key="http_turn_single_anchor",
            kind=proxy_service.StickySessionKind.CODEX_SESSION,
        ),
    )
    account = cast(Any, SimpleNamespace(id="acc-only", status=AccountStatus.ACTIVE))
    ...
    session = proxy_service._HTTPBridgeSession(
        key=proxy_service._HTTPBridgeSessionKey("session_header", "sid-single-anchor", None),
        headers={"x-codex-session-id": "sid-single-anchor", "x-codex-turn-state": "http_turn_single_anchor"},
        affinity=request_state.affinity_policy,
        request_model="gpt-5.5",
        account=account,
        upstream=upstream,
        ...
        last_used_at=1.0,
        idle_ttl_seconds=120.0,
        upstream_turn_state="http_turn_single_anchor",
        downstream_turn_state="http_turn_single_anchor",
    )
    settings = SimpleNamespace(
        prefer_earlier_reset_accounts=False,
        prefer_earlier_reset_window="secondary",
    ...
    monkeypatch.setattr(service, "_select_account_with_budget_for_stream", select_account)
    monkeypatch.setattr(service, "_ensure_fresh_with_budget", AsyncMock(side_effect=lambda selected, **_: selected))
    monkeypatch.setattr(service, "_open_upstream_websocket_with_budget", AsyncMock(return_value=upstream))
    monkeypatch.setattr(
        service,
        "_acquire_account_response_create_lease_or_overload",
