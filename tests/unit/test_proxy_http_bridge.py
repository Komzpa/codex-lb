...
@pytest.mark.asyncio
async def test_stream_http_bridge_session_events_preserves_recovery_signal_for_masked_previous_response(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    request_state = proxy_service._WebSocketRequestState(
        request_id="req-prev-miss-http-json",
        model="gpt-5.4",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=1.0,
        previous_response_id="resp_missing_http_json",
        event_queue=asyncio.Queue(),
        transport="http",
        skip_request_log=True,
    )
    ...
    assert "resp_missing_http_json" not in json.dumps(exc_info.value.payload)
...
async def test_process_http_bridge_upstream_text_masks_previous_response_usage_limit(
    ...
    request_state = proxy_service._WebSocketRequestState(
        ...
        request_text=(
            ...
            '"previous_response_id":"resp_owner_only","input":"follow-up"}'
        ),
        transport="http",
        skip_request_log=True,
    )
...
@pytest.mark.asyncio
async def test_submit_http_bridge_request_fails_locally_for_known_lost_previous_response(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    service = proxy_service.ProxyService(cast(Any, nullcontext()))
    send_text = AsyncMock()
    ...
    session = proxy_service._HTTPBridgeSession(
        ...
        last_used_at=time.monotonic(),
        idle_ttl_seconds=120.0,
        lost_previous_response_ids={"resp_lost_anchor"},
    )
    request_state = proxy_service._WebSocketRequestState(
        ...
        event_queue=asyncio.Queue(),
        transport="http",
        previous_response_id="resp_lost_anchor",
        request_text='{"type":"response.create","previous_response_id":"resp_lost_anchor"}',
    )
    ...
    assert failed_event is not None
    assert '"code":"stream_incomplete"' in failed_event
    assert "previous_response_not_found" not in failed_event
    assert await asyncio.wait_for(event_queue.get(), timeout=0.1) is None
    send_text.assert_not_awaited()
