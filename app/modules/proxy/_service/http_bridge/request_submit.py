    ...
    async def _retry_http_bridge_precreated_request(self: Any, session: "_HTTPBridgeSession") -> bool:
        async with session.pending_lock:
            ...
            migrate_stalled_owner = bool(
                ...
                or (request_state.previous_response_id is None and not request_state.file_required_preferred_account)
            )
            ...
            # into the client's original full resend. An unanchored, file-free
            # request is likewise safe to move. Keeping the prior preferred
            # owner would reconnect to the same account that just failed to
            # create a response, defeating the startup retry.
